...
//-------------------------------------------------------------------------------------
#define DATA_FILE                                                              \
  "data.bin" // The name of the file which is storing acquired data.
#define ACQUISITION_DURATION_S 25.0f // The acquisition duration in seconds.
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if (availableDevicesCount < 1) {
      std::cout
          << "No device available. Please pair with a Unicorn device first.";
      errorCode = UNICORN_ERROR_GENERAL_ERROR;
    }

    // Get available device serials.
    UNICORN_DEVICE_SERIAL *availableDevices =
        new UNICORN_DEVICE_SERIAL[availableDevicesCount];
    ...
    //-------------------------------------------------------------------------------------
    std::cout << "Trying to connect to '" << availableDevices[deviceSelection]
              << "'." << std::endl;
    errorCode =
    ...
    try {
      ...
      std::cout << "Number Of Acquired Channels: " << numberOfChannelsToAcquire
                << std::endl;
      std::cout << "Data Acquisition Length: " << ACQUISITION_DURATION_S << "s"
                << std::endl;
      ...
      int consoleUpdateRate =
          (int)((UNICORN_SAMPLING_RATE / FRAME_LENGTH) / 25.0f);
      ...
      for (int i = 0; i < numberOfGetDataCalls; i++) {
        // Receives the configured number of samples from the Unicorn device and
        // writes it to the acquisition buffer.
        errorCode =
            UNICORN_GetData(deviceHandle, FRAME_LENGTH, acquisitionBuffer,
                            acquisitionBufferLength * sizeof(float));
        HandleError(errorCode);

        // Write data to file.
        file.write((const char *)acquisitionBuffer,
                   acquisitionBufferLength * sizeof(float));
...
void PrintErrorMessage(unsigned int errorCode) {
  std::cout << std::endl
            << "An error occurred. Error Code: " << errorCode << " - ";
  switch (errorCode) {
  ...
  case UNICORN_ERROR_BLUETOOTH_SOCKET_FAILED:
    std::cout << "The operation could not be performed because the Bluetooth "
                 "socket failed.";
    break;
