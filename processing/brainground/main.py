...
class FrequencyBand():
    def __init__(self, name: str, start: float, end: float, has_checkbox: bool = True):
        self.name = name
        self.start = start
        self.end = end
    ...
    def checkbox_callback(self):
        self.active = self.checkbox.isChecked()

class Channel():
    def __init__(self, id: int):
        self.id = id
        self.checkbox = QtWidgets.QCheckBox(f"Ch {id}")
        self.checkbox.stateChanged.connect(self.checkbox_callback)
        self.active = True
...
class LSLTimeScope(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        for band in self.frequency_bands:
            controls_layout.addWidget(band.checkbox)
    ...
    # TODO: think of a nicer name for this (lol)
    ...
    def update_plot(self):
        ...
        if self.psd_avg_computed:
            ...
            for i, band in enumerate(self.frequency_bands):
                ...
                # print(f"band_avg [{band.name}]: {self.psd_avg[i]}")
                # print(f"band_delta [{band.name}]: {delta}")
                # print(f"band_curr [{band.name}]: {power_per_band[i]}")
